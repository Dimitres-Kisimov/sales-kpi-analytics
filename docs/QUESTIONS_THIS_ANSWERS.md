...
| 4 | Did we grow by selling more or by discounting (price vs volume vs mix)? | `saleskpi.metrics.margin_bridge` → `price_effect_eur`, `volume_effect_eur`, `mix_effect_eur` | Mix is the residual so the three effects sum exactly to the total change; needs 24 months (last-12 vs prior-12). |
