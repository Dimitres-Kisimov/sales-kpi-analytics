...
| 16 | Which KPIs went out of control this year, and when? | `saleskpi.anomaly.kpi_exceptions` → `alerts` (rendered as `kpi_control_chart.svg`) | Robust control limits (median ± 3.5·MAD/0.6745, Iglewicz–Hoaglin); monitors the quality/service ratios + AOV. Volume series (revenue, orders) are excluded — 24 points are too few for seasonal level-anomaly detection. |
| 17 | Are we on track to hit the annual plan, and what will we land at? | `saleskpi.pacing.target_attainment` → `projected_full_year_eur`, `projected_attainment_pct`, `interval_low_eur` / `interval_high_eur` (rendered as `pacing_bullet.svg`) | Plan = prior-FY revenue × (1 + 8% **assumed**); the projection uses the rolling-origin-CV-selected model and the interval is its own empirical backtest error (normal-approx, independence across months) — modelled, **not a guarantee**. The realised full year is reported as a closed-year back-check. |
